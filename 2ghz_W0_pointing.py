...
def main():
    print("\n" + "="*60)
    print("   2-GHZ QKD Specific Simulation: Altitude, PE, & W0 Sweep")
    print("="*60)
    ...
    angles = [FIXED_ANGLE] * M_USERS

    for alt in alt_list:
        print(f"\n[{'='*20} HAP Altitude: {alt} km {'='*20}]")
        
        for pe in pe_list:
            print(f"  [{'*'*15} Pointing Error: {pe} {'*'*15}]")
            
            for w0 in w0_list:
                print(f"    --- Beam Waist W0 = {w0:.3f} m ---")
                ...
                print(f"        ├─ System SKR       : {skr_sys_uni:10.5f} bps")
                print(f"        ├─ Total Exec Time  : {t_total:10.4f} sec")
                print(f"        └─ Total Steps      : {num_groups * 4} steps")
