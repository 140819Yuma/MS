...
def create_balanced_star_groups(n_users, angles, group_size):
    ...
    while len(others) > 0:
        if len(others) >= group_size - 1:
            members = [leader]
            for i in range(group_size - 1): members.append(others.pop(0)[0] if i % 2 == 0 else others.pop(-1)[0])
            groups.append(sorted(members))
        else:
            members = [leader]; leftover = [x[0] for x in others]; members.extend(leftover); others.clear()
...
def main():
    print("=== Scalability Full Qiskit Simulation: Users 5 to 25 ===")
    ...
    for n_users in user_counts:
        ...
        for size in ghz_sizes:
            ...
            groups = create_balanced_star_groups(n_users, angles, size)
            group_skrs = []
    ...
    print("\n\n" + "="*65)
    print(" SUMMARY: Full Qiskit System Throughput (Mbps)")
    print("="*65)
