...
FREQ = 1e9; PULSES_PER_SLOT = 10**5; BATCH_SIZE = 10**6
FIXED_Q_X = 0.02
P_Z_ALL = 0.9  # 2-GHZ, 3-GHZともにZ基底を選ぶ確率を90%に固定
...
# ==========================================
# 今回の指定条件 (W0 と ユーザー数のリスト)
# ==========================================
w0_list = [0.05, 0.10, 0.15, 0.20]
user_counts = [6, 12, 18, 24, 30]
...
def main():
    print("="*70)
    print(" === HAP-Alice: Scalability Simulation (2-GHZ & 3-GHZ) ===")
    print(" === Sweep Parameters: Beam Waist (W0) & Users (M)     ===")
    print("="*70)
    ...
    for w0 in w0_list:
        print(f"\n[==================== Beam Waist W0: {w0:.2f} m ====================]")
        
        for N_USERS in user_counts:
            print(f"  [********** Number of Users: {N_USERS:2d} (Angle: {FIXED_ANGLE} deg) **********]")
            
            # W0を反映したチャネルモデルの生成
            eta_matrix = custom_generate_realistic_eta(N_USERS, NUM_TIMESLOTS, [FIXED_ANGLE] * N_USERS, current_w0=w0)
            eta_alice = 1.0
                ...
                for g in range(num_groups):
                    ...
                    for ts in range(NUM_TIMESLOTS):
                        ...
                        if n_ev > 0:
                            ...
                            # 高速化のために GLOBAL_BACKEND を渡す
                            ev, er = run_full_qkd_protocol_qiskit(n_ghz, batch, max_q, P_Z_ALL, GLOBAL_BACKEND)
                            z_ev += ev * (n_ev / batch)
                            z_er += er * (n_ev / batch)
